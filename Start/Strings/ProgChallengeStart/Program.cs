...
namespace ProgChallengeStart
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // Use a do-while loop becaue we know we want it to execute at least once 
            do
            {
                // Ask user for current guess
                Console.WriteLine("What is your guess?");
                string theGuess = Console.ReadLine();

                // Use TryParse method to parse the input into a number
                ...
                {
                    // If they enter a -1 then exit the game
                    if (guessNum == -1)
                    {
